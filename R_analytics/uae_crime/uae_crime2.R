...
## Load Data
#===================
crimes_data <- read_excel("dataset crimes UAE.xlsx")

crimes_data <- crimes_data %>%
  mutate(Age = as.numeric(Age)) %>% # Convert age to numerical
  dplyr::select(-'Description of the charge') %>% # Remove charge description
...
crime_info2 <- crimes_data_model %>%
  select(Gender, `Marital Status`, `Educational Level`, Religion,Job, Emirate,Nationality_en)
